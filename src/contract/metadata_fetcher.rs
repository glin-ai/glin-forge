...
pub async fn fetch_contract_metadata(
    ...
    if let Some(path) = options.local_path {
        println!("{} Loading metadata from: {}", "→".cyan(), path);
    ...
    if let Some(cache_dir) = &options.cache_dir {
        if let Ok(metadata) = try_load_from_cache(cache_dir, contract_address) {
            println!("{} Loaded metadata from cache", "✓".green());
            return Ok(metadata);
        }
    }

    // Strategy 3: Get code hash from blockchain
    println!("{} Fetching contract info from chain...", "→".cyan());
    let code_hash_hex = match crate::contract::chain_info::get_contract_info(client, contract_address).await {
        Ok(info) => {
            let hash = format!("0x{}", hex::encode(&info.code_hash));
            println!("  {} Code hash: {}", "→".cyan(), hash);
            Some(hash)
        }
        Err(e) => {
            eprintln!("  {} Failed to get code hash from chain: {}", "⚠".yellow(), e);
            None
        }
    };
    ...
    if let Some(explorer_url) = options.explorer_url {
        println!("{} Fetching metadata from explorer...", "→".cyan());

        match fetch_from_explorer(&explorer_url, contract_address, code_hash_hex.as_deref()).await {
            Ok(metadata) => {
                println!("{} Successfully fetched from explorer", "✓".green());

                // Cache it for future use
                if let Some(cache_dir) = &options.cache_dir {
                    if let Err(e) = save_to_cache(cache_dir, contract_address, &metadata) {
                        eprintln!("  {} Failed to cache metadata: {}", "⚠".yellow(), e);
                    } else {
                        println!("  {} Cached metadata for future use", "✓".green());
                    }
                }
            ...
            Err(e) => {
                eprintln!("  {} Explorer fetch failed: {}", "⚠".yellow(), e);
            }
        }
    }
...
1. Specify metadata file:
   {} --metadata path/to/contract.json

2. Use an explorer with verification:
   {} --explorer https://explorer.example.com

3. Place metadata in cache directory:
   ~/.glin-forge/cache/{}.json

For more info, see: https://use.ink/basics/metadata
"#,
        contract_address,
        "glin-forge <command>".yellow(),
        "glin-forge <command>".yellow(),
        contract_address
    ))
}
